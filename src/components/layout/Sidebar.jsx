...
export default function Sidebar({ activeTab, onNavigate, onLogout, isOpen }) {
  ...
  return (
    <div className={`${isOpen ? 'w-64' : 'w-20'} bg-white border-r border-gray-200 h-screen flex flex-col fixed left-0 top-0 z-50 transition-all duration-300 ease-in-out shadow-sm`}>
      ...
      <div className={`h-20 flex items-center px-6 border-b border-gray-100 ${isOpen ? 'gap-3' : 'justify-center'}`}>
        <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white shadow-lg shadow-blue-200 shrink-0 transition-transform hover:scale-105">
          <Building2 className="w-6 h-6" />
        </div>
        {/* Sidebar açıksa yazıyı göster, kapalıysa gizle */}
        <div className={`overflow-hidden transition-all duration-300 ${isOpen ? 'opacity-100 w-auto' : 'opacity-0 w-0 hidden'}`}>
          <h1 className="text-xl font-bold text-gray-800 tracking-tight whitespace-nowrap">Paylora</h1>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest whitespace-nowrap">HR SYSTEM</p>
        </div>
      </div>
        ...
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              title={!isOpen ? item.label : ''} // Kapalıyken tooltip çıksın
      ...
      <div className="p-4 border-t border-gray-100 bg-gray-50/30">
        <button 
          onClick={onLogout}
          className={`w-full flex items-center justify-center rounded-xl text-sm font-bold text-red-600 bg-red-50 hover:bg-red-100 hover:text-red-700 transition-colors
